...
Lcd.prototype.print = function print(str, cb) {
  var self = this;
  var arrayToPrint = [0x40];

  for(var i = 0; i < str.length; i++) {
    arrayToPrint.push(str.charCodeAt(i));
  }

  self.wire.write(arrayToPrint, function(err) {
      if(err) {
        return cb(err);
      }
      else {
 ...
 */
Lcd.prototype.clrscr = function clrscr(cb) {
  var self = this;
  self.wire.write([0x00, CMD_CLEARSCREEN], function(err) {
    if(err) {
      return cb(err);  
    }
    else {
